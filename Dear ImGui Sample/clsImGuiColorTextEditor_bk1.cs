...
namespace WinOpenTK_ImGUI
{
	...
	class clsImGuiColorTextEditor
	{
		...
		private int intAnchorEndLine = -1;
		private uint intCursorPosition = 0;
		public Func<Vector4> BackColor { get; set; } = () =>
		{ Vector4 col = ImGui.ColorConvertU32ToFloat4(ImGui.GetColorU32(ImGuiCol.FrameBg));
		  col.W = 0.25f; return col; };
		...
		public static (uint Idx, int EndLineAnchor) GetCursorPositionFromPos(infoLine[] aryLines, Vector2 pos)
		{
			return GetCursorPositionFromLocation(aryLines, GetLocationFromPosition(aryLines, pos));
		}
		public static (Point Loc, uint Idx) GetLocationFromCursorPos(infoLine[] aryLines, uint pos, int EndLineAnchor)
		{
				...
				if (pos >= aryLines[itr].PositionStart)
				{
					...
					else
					{
						if (itr < aryLines.Length - 1)
						{
							...
							if (itr == EndLineAnchor)
							{
								if (pos > aryLines[itr].PositionEnd)
								{
									return (new Point((int)aryLines[itr].LineLength, itr), aryLines[itr].PositionEnd + 1);
								}
							}
						}
						else
		...
		public static (uint idxPos, int EndLineAnchor) GetCursorPositionFromLocation(infoLine[] aryLines, Point pos)
		{
			if (pos.Y < 0) pos.Y = 0;
			if (pos.Y >= aryLines.Length) pos.Y = aryLines.Length - 1;
			infoLine ln = aryLines[pos.Y];
			char charEndLine = ' '; if (pos.X >= 0 && pos.X < ln.ValueLength) charEndLine = ln.Value[pos.X];
			int intRet = (pos.X >= 0 && pos.X < ln.ValueLength) ?(pos.Y):(-1);
			if(intRet == -1)
			{
				if (pos.X >= aryLines[pos.Y].LineLength)
				{
					if(pos.Y < aryLines.Length - 1)
					{
						pos.X = Math.Max(0, pos.X - ((int)aryLines[pos.Y].LineLength));
						pos.Y = Math.Min(aryLines.Length - 1, pos.Y + 1);
					}
					else 
					{
						pos.X = Math.Max(0, (int)aryLines[pos.Y].LineLength);
					}
				}
			}
			else
			{
				if (pos.X >= aryLines[pos.Y].LineLength) pos.X = Math.Max(0, (int)aryLines[pos.Y].LineLength);
			}
			return ((uint)(aryLines[pos.Y].PositionStart + pos.X), intRet);
		}
		...
		public uint CursorPosition
		{
			...
			set { intCursorPosition = GetLocationFromCursorPos(Lines, value, intAnchorEndLine).Idx; }
		}
		public Point CursorLocation
		{
			get => GetLocationFromCursorPos(Lines, intCursorPosition, intAnchorEndLine).Loc;
			set { (CursorPosition, intAnchorEndLine) = GetCursorPositionFromLocation(Lines, value); }
		}
		...
		public infoLine[] SelectedLines
		{
			get
			{
				...
				Point locStart = GetLocationFromCursorPos(aryLines, posStart, -1).Loc;
				Point locEnd = GetLocationFromCursorPos(aryLines, posEnd, -1).Loc;
		...
		public void UpdateInputState(clsInputState ioState)
		{
			...
			if (bolIsHovered)
			{
				...
				int EndLineAnchor = -1;
				uint intMouseContent; (intMouseContent, EndLineAnchor) = GetCursorPositionFromLocation(Lines, locMouseContent);
				   ...
				   posMouse.Y >= posWindow.Y && posMouse.Y <= endWindow.Y)
				{
					...
					if (ImGui.IsMouseClicked(ImGuiMouseButton.Left))
					{
						...
						{
							SelectionLength = 0;
							intAnchorEndLine = EndLineAnchor;
			...
			if (bolIsActive)
			{
				...
				if (ioState.KeyboardState[(int)Keys.Right]) { ioState.KeyboardState[(int)Keys.Right] = false; CursorPosition++; bolCursorUpdate = true; }
				if (ioState.KeyboardState[(int)Keys.Left]) { ioState.KeyboardState[(int)Keys.Left] = false; CursorPosition--; bolCursorUpdate = true; }
				...
				if(strInputChars.Length > 0)
				{
					...
					foreach (char c in strInputChars)
					{
						if (c >= 32)
						{
							...
							if (Lines[CursorLine].LineWrapIndex != -1) intAnchorEndLine = -1;
							CursorPosition++;
							bolCursorUpdate = true;
						}
					}
		...
		public void Render()
		{
			...
			for (int itrLine = 0; itrLine < arySelectedLines.Length; itrLine++)
			{
				...
				Point locLine = GetLocationFromCursorPos(aryLines, intStart, -1).Loc;
				Vector2 posLineRectMin = GetPositionFromCursorPos(aryLines, locLine) - posContent;
				locLine = GetLocationFromCursorPos(aryLines, intEnd, -1).Loc;
				...
				if (intRow < dimChars.Height && posScr.Y >= posWindow.Y - posMove.Y && posScr.Y <= endWindow.Y)
				{
					...
					while (posScr.X <= posWindow.X + szWindow.X)
					{
						...
						if (posCursor.X == intColumn && posCursor.Y == intRow)
						{
							...
							{
								draw.AddTriangleFilled(posScr + new Vector2(-2, -5), posScr + new Vector2(6, -5), posScr + new Vector2(2, 0), intColorFore);
								draw.AddRectFilled(posScr + new Vector2(1, 0), posScr + new Vector2(2, posMove.Y - 4), intColorFore);
							}
						}
						posScr.X += posMove.X;
						intColumn++;
